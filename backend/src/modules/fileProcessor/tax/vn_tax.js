// backend/src/modules/fileProcessor/parsers/vn_tax.js
/**
 * 越南 VAT 税务计算
 */
...
function calculateVAT(amount, rate = VAT_RATES.STANDARD) {
    if (rate === VAT_RATES.EXEMPT) {
        return { netAmount: amount, vatAmount: 0, grossAmount: amount, rate: 'exempt', currency: 'VND' };
    }
    const vatAmount = (amount * rate) / 100;
    return { netAmount: amount, vatAmount: vatAmount, grossAmount: amount + vatAmount, rate: rate, currency: 'VND' };
}

function validateVATNumber(vatNumber) {
    const pattern = /^\d{10}$/;
    if (!pattern.test(vatNumber)) return { valid: false, message: '无效的越南 VAT 号码格式' };
...
function getVATSummary(transactions) {
    ...
    return { ...summary, currency: 'VND', country: 'VN', countryName: '越南' };
}

module.exports = {
    VAT_RATES, calculateVAT, validateVATNumber, getVATSummary,
    countryCode: 'VN', countryName: '越南', currency: 'VND', taxSystem: 'VAT'
};