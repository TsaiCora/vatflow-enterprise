// backend/src/modules/fileProcessor/parsers/dk_tax.js
/**
 * 丹麦 VAT 税务计算
 */
...
function calculateVAT(amount, rate = VAT_RATES.STANDARD) {
    if (rate === VAT_RATES.EXEMPT) {
        return { netAmount: amount, vatAmount: 0, grossAmount: amount, rate: 'exempt', currency: 'DKK' };
    }
    const vatAmount = (amount * rate) / 100;
    return { netAmount: amount, vatAmount: vatAmount, grossAmount: amount + vatAmount, rate: rate, currency: 'DKK' };
}

function validateVATNumber(vatNumber) {
    const pattern = /^DK\d{8}$/;
    if (!pattern.test(vatNumber)) return { valid: false, message: '无效的丹麦 VAT 号码格式' };
...
function getVATSummary(transactions) {
    ...
    return { ...summary, currency: 'DKK', country: 'DK', countryName: '丹麦' };
}

module.exports = {
    VAT_RATES, calculateVAT, validateVATNumber, getVATSummary,
    countryCode: 'DK', countryName: '丹麦', currency: 'DKK', taxSystem: 'VAT'
};