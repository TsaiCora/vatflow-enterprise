// backend/src/modules/fileProcessor/parsers/ph_tax.js
/**
 * 菲律宾 VAT 税务计算
 */
...
function calculateVAT(amount, rate = VAT_RATES.STANDARD) {
    if (rate === VAT_RATES.EXEMPT) {
        return { netAmount: amount, vatAmount: 0, grossAmount: amount, rate: 'exempt', currency: 'PHP' };
    }
    const vatAmount = (amount * rate) / 100;
    return { netAmount: amount, vatAmount: vatAmount, grossAmount: amount + vatAmount, rate: rate, currency: 'PHP' };
}

function validateVATNumber(vatNumber) {
    const pattern = /^\d{12}$/;
    if (!pattern.test(vatNumber)) return { valid: false, message: '无效的菲律宾 VAT 号码格式' };
...
function getVATSummary(transactions) {
    ...
    return { ...summary, currency: 'PHP', country: 'PH', countryName: '菲律宾' };
}

module.exports = {
    VAT_RATES, calculateVAT, validateVATNumber, getVATSummary,
    countryCode: 'PH', countryName: '菲律宾', currency: 'PHP', taxSystem: 'VAT'
};