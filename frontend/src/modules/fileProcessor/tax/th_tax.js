// backend/src/modules/fileProcessor/parsers/th_tax.js
/**
 * 泰国 VAT 税务计算
 */
...
function calculateVAT(amount, rate = VAT_RATES.STANDARD) {
    if (rate === VAT_RATES.EXEMPT) {
        return { netAmount: amount, vatAmount: 0, grossAmount: amount, rate: 'exempt', currency: 'THB' };
    }
    const vatAmount = (amount * rate) / 100;
    return { netAmount: amount, vatAmount: vatAmount, grossAmount: amount + vatAmount, rate: rate, currency: 'THB' };
}

function validateVATNumber(vatNumber) {
    const pattern = /^\d{13}$/;
    if (!pattern.test(vatNumber)) return { valid: false, message: '无效的泰国 VAT 号码格式' };
...
function getVATSummary(transactions) {
    ...
    return { ...summary, currency: 'THB', country: 'TH', countryName: '泰国' };
}

module.exports = {
    VAT_RATES, calculateVAT, validateVATNumber, getVATSummary,
    countryCode: 'TH', countryName: '泰国', currency: 'THB', taxSystem: 'VAT'
};