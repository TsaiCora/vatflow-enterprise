// backend/src/modules/fileProcessor/parsers/za_tax.js
/**
 * 南非 VAT 税务计算
 */
...
function calculateVAT(amount, rate = VAT_RATES.STANDARD) {
    if (rate === VAT_RATES.EXEMPT) {
        return { netAmount: amount, vatAmount: 0, grossAmount: amount, rate: 'exempt', currency: 'ZAR' };
    }
    const vatAmount = (amount * rate) / 100;
    return { netAmount: amount, vatAmount: vatAmount, grossAmount: amount + vatAmount, rate: rate, currency: 'ZAR' };
}

function validateVATNumber(vatNumber) {
    const pattern = /^\d{10}$/;
    if (!pattern.test(vatNumber)) return { valid: false, message: '无效的南非 VAT 号码格式' };
...
function getVATSummary(transactions) {
    ...
    return { ...summary, currency: 'ZAR', country: 'ZA', countryName: '南非' };
}

module.exports = {
    VAT_RATES, calculateVAT, validateVATNumber, getVATSummary,
    countryCode: 'ZA', countryName: '南非', currency: 'ZAR', taxSystem: 'VAT'
};